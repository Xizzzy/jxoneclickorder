...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#voucher").autocomplete(
            '{$link->getAdminLink('AdminCartRules')|addslashes|escape:'htmlall':'UTF-8'}',
            {
                ...
                parse       : function(data) {
                    if (!data.found) {
                        $('#vouchers_err').html('{l s='No voucher was found' mod='jxoneclickorder'}').show();
                    } else {
                        $('#vouchers_err').hide();
                    }
                    var mytab = new Array();
                ...
                $.ajax({
                    ...
                    success          : function(res) {
                        if (res.result) {
                            ...
                            console.log($('.selected_customer .customer_firstname').text());
                            $('#address_part #firstname').val($('.selected_customer .customer_firstname').text());
                            $('#address_part #lastname').val($('.selected_customer .customer_lastname').text());
                        }
                    }
                });
            });
        ...
        $d.off('click', 'button[name=submitAddOrder]').on('click', 'button[name=submitAddOrder]', function(e) {
            ...
            var el            = $(this),
                ajax_settings = {
                    data    : {
                        id_cart             : id_cart,
                        payment_module_name : $('#payment_module_name').val(),
                        id_order_state      : $('#id_order_state').val(),
                ...
                ajax_settings = {
                    ...
                    success : function(msg) {
                        if (msg.status) {
                            ...
                            $('#address_delivery, #address_invoice').val(msg.id_address).show();
                            $('#addresses_err').hide();
                            updateAddresses();
            ...
            success  : function(res) {
                if (res.found) {
                    ...
                    $.each(res.customers, function() {
                        ...
                        html += '<button type="button" data-customer="' + this.id_customer + '" class="setup-customer btn btn-default pull-right"><i class="icon-arrow-right"></i> {l s='Choose' mod='jxoneclickorder'}</button>';
                        html += '</div>';
                        html += '</div>';
                    });
                }
                else
                    html = '<div class="alert alert-warning">{l s='No customers found' mod='jxoneclickorder'}</div>';
                $('#customers').html(html);
            }
        });
    }
            ...
            success  : function(res) {
                if (res.found) {
                    ...
                    $.each(res.carts, function() {
                        ...
                        html_carts += '<a title="{l s='View this cart' mod='jxoneclickorder'}" class="btn btn-default" href="index.php?tab=AdminCarts&id_cart=' + this.id_cart + '&viewcart&token={getAdminToken tab='AdminCarts'}&liteDisplaying=1#"><i class="icon-search"></i>&nbsp;{l s='Details' mod='jxoneclickorder'}</a>';
                        html_carts += '&nbsp;<a href="#" title="{l s='Use this cart' mod='jxoneclickorder'}" class="use_cart btn btn-default" rel="' + this.id_cart + '"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='jxoneclickorder'}</a>';
                        html_carts += '</td>';
                        html_carts += '</tr>';
                    });
                    $.each(res.orders, function() {
                        ...
                        html_orders += '<a href="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&id_order=' + this.id_order + '&vieworder&liteDisplaying=1#" title="{l s='View this order' mod='jxoneclickorder'}" class="btn btn-default"><i class="icon-search"></i>&nbsp;{l s='Details' mod='jxoneclickorder'}</a>';
                        html_orders += '&nbsp;<a href="#" "title="{l s='Duplicate this order' mod='jxoneclickorder'}" class="duplicate_order btn btn-default" rel="' + this.id_order + '"><i class="icon-arrow-right"></i>&nbsp;{l s='Use' mod='jxoneclickorder'}</a>';
                        html_orders += '</td>';
                        html_orders += '</tr>';
                    });
    ...
    function updateDeliveryOptionList(delivery_option_list) {
        ...
        if (delivery_option_list.length > 0) {
            ...
            $('#carrier_form').hide();
            $('#carriers_err').show().html('{l s='No carrier can be applied to this order' mod='jxoneclickorder'}');
        ...
        $.ajax({
            ...
            success  : function(res) {
                ...
                if (res.found) {
                    ...
                    $('#products_found').show();
                    products_found += '<label class="control-label col-lg-3">{l s='Product' mod='jxoneclickorder'}</label><div class="col-lg-6"><select id="id_product" onclick="display_product_attributes();display_product_customizations();"></div>';
                    attributes_html += '<label class="control-label col-lg-3">{l s='Combination' mod='jxoneclickorder'}</label><div class="col-lg-6">';
                    $.each(res.products, function() {
                        ...
                        if (this.customizable == '1' || this.customizable == '2') {
                            $('#products_part').closest('.form-horizontal').prepend('<form id="customization_' + id_product + '" class="id_customization" method="post" enctype="multipart/form-data" action="' + admin_cart_link + '" style="display:none;"></form>');
                            customization_html += '<div class="bootstrap"><div class="panel"><div class="panel-heading">{l s='Customization' mod='jxoneclickorder'}</div><div id="customization_' + id_product + '" class="id_customization" style="display:none;">';
                            customization_html += '<input type="hidden" form="customization_' + id_product + '" name="id_product" value="' + id_product + '" />';
                            customization_html += '<input type="hidden" form="customization_' + id_product + '" name="id_cart" value="' + id_cart + '" />';
                            customization_html += '<input type="hidden" form="customization_' + id_product + '" name="action" value="updateCustomizationFields" />';
                            customization_html += '<input type="hidden" form="customization_' + id_product + '" name="id_customer" value="' + id_customer + '" />';
                            customization_html += '<input type="hidden" form="customization_' + id_product + '" name="ajax" value="1" />';
                            $.each(this.customization_fields, function() {
                                ...
                                if (this.type == 0)
                                    customization_html += '<input form="customization_' + id_product + '" class="form-control customization_field" type="file" name="customization_' + id_product + '_' + this.id_customization_field + '" id="customization_' + id_product + '_' + this.id_customization_field + '">';
                                else if (this.type == 1)
                                    customization_html += '<input form="customization_' + id_product + '" class="form-control customization_field" type="text" name="customization_' + id_product + '_' + this.id_customization_field + '" id="customization_' + id_product + '_' + this.id_customization_field + '">';
                    ...
                    $('#products_found').hide();
                    $('#products_err').html('{l s='No products found' mod='jxoneclickorder'}');
    ...
    function display_product_customizations() {
        if ($('#products_found #customization_list').find('#customization_' + $('#id_product option:selected').val()).children().length === 0)
            $('#customization_list').hide();
        else {
            $('#customization_list').show();
            $('#products_found #customization_list').find('.id_customization').hide();
            $('#products_found #customization_list').find('#customization_' + $('#id_product option:selected').val()).show();
            $('#products_found #customization_list').css('height', $('#products_found #customization_list').find('#customization_' + $('#id_product option:selected').val()).height() + 95 + 'px');
        }
    }
    ...
    function updateCartProducts(products, gifts, id_address_delivery) {
        ...
        $.each(products, function() {
            ...
            formatCurrencyCldr(this.numeric_total, function(e){
              cart_content += '</td><td>' +e+ '</td></tr>';
            });
        ...
        $.each(gifts, function() {
            ...
            cart_content += '<td>{l s='Gift' mod='jxoneclickorder'}</td><td>' + this.cart_quantity + '</td><td>{l s='Gift' mod='jxoneclickorder'}</td></tr>';
        });
        ...
        if (typeof(vouchers) == 'object')
            $.each(vouchers, function() {
                if (parseFloat(this.value_real) === 0 && parseInt(this.free_shipping) === 1)
                    var value = '{l s='Free shipping' mod='jxoneclickorder'}';
                else
                    var value = this.value_real;
                vouchers_html += '<tr><td>' + this.name + '</td><td>' + this.description + '</td><td>' + value + '</td><td class="text-right"><a href="#" class="btn btn-default delete_discount" rel="' + this.id_discount + '"><i class="icon-remove text-danger"></i>&nbsp;{l s='Delete' mod='jxoneclickorder'}</a></td></tr>';
            });
        ...
        $.ajax({
            ...
            success  : function(res) {
                ...
                if (res.errors.length) {
                    ...
                    updatePanelStatus($('products_part'), 'success');
                    showSuccessMessage('{l s='Product successfully added' mod='jxoneclickorder'}');
                    $('#products_err').addClass('hide');
                }
    ...
    function submitCustomizations(form) {
    ...
    function addProduct() {
        ...
        submitCustomizations($('#customization_' + id_product));
        addProductProcess();
    }
    ...
    function updateAddressesList(addresses, id_address_delivery, id_address_invoice) {
        ...
        if (addresses.length == 0) {
            ...
            $('#addresses_err').show().html('{l s='You must add at least one address to process the order.' mod='jxoneclickorder'}');
...
<h3 class="order_header">
    {l s='Preorder' mod='jxoneclickorder'} #{$order->id_order|escape:'htmlall':'UTF-8'}
    ...
    <div class="panel form-horizontal">
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                <i class="icon-info"></i>
                {l s='Preorder information' mod='jxoneclickorder'}
            </div>
...
<div class="panel form-horizontal closed {if $cart->id_customer != 0}success{else}warning{/if}" id="customer_part">
    <div class="panel-heading toggle-panel">
        <div class="panel-title">
            <i class="icon-user"></i>
            {l s='Customer' mod='jxoneclickorder'}
        </div>
        <a href="#" class="toggle-panel-btn">
    ...
    <div class="panel-content" style="display:none">
        <div class="create_customer col-sm-6 {if $cart->id_customer != 0}hidden{/if}">
            <h4>{l s='Create customer' mod='jxoneclickorder'}</h4>
            <form method="post" class="std">
                <div class="account_creation">
                    <div class="errors"></div>
                    <div class="required form-group">
                        <label for="customer_firstname">{l s='First name' mod='jxoneclickorder'} <sup>*</sup></label>
                    ...
                    <div class="required form-group">
                        <label for="customer_lastname">{l s='Last name' mod='jxoneclickorder'} <sup>*</sup></label>
                    ...
                    <div class="required form-group">
                        <label for="email">{l s='Email' mod='jxoneclickorder'} <sup>*</sup></label>
                    ...
                    <div class="required password form-group">
                        <label for="passwd">{l s='Password' mod='jxoneclickorder'}<sup>*</sup></label>
                        <div class="input_wrapper">
                            <input type="text" class="is_required validate form-control" data-validate="isPasswd"
                                   name="passwd"/>
                            <div class="btn btn-default" id="random_password">{l s='Random' mod='jxoneclickorder'}</div>
                        </div>
                        <span class="form_info">{l s='(Five characters minimum)' mod='jxoneclickorder'}</span>
                    </div>
                    <a href="#" class="btn btn-default pull-left" id="create_random_customer"
                       data-id-order="{$order->id_order|escape:'htmlall':'UTF-8'}">{l s='Create random customer' mod='jxoneclickorder'}</a>
                    <a href="#" class="btn btn-default pull-right" id="create_customer"
                       data-id-order="{$order->id_order|escape:'htmlall':'UTF-8'}">{l s='Create and choose customer' mod='jxoneclickorder'}</a>
                </div>
            </form>
        </div>
        <div class="select_customer col-sm-6 {if $cart->id_customer != 0}hidden{/if}">
            <h4>{l s='Select customer' mod='jxoneclickorder'}</h4>
            <div class="form-group" id="search-customer-form-group">
                <label>
                    {l s='Search for a customer' mod='jxoneclickorder'}
                </label>
        ...
        <div class="selected_customer col-sm-12 {if $cart->id_customer == 0}hidden{/if}">
            {if $cart->id_customer != 0}
                {include './../_partials/customer.tpl'}
            {/if}
        </div>
        <div class="panel-buttons col-sm-12">
            <a class="btn btn-default btn-success pull-right other-customer {if $cart->id_customer == 0}hidden{/if}">{l s='Select another customer' mod='jxoneclickorder'}</a>
            <a href="#" class="btn btn-default pull-right cancel hidden">{l s='Cancel' mod='jxoneclickorder'}</a>
        </div>
    </div>
</div>
...
<form class="form-horizontal"
    ...
    <div class="panel closed success">
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                <i class="icon-usd"></i>
                {l s='Languages and currencies' mod='jxoneclickorder'}
            </div>
            <a href="#" class="toggle-panel-btn">
                <span class="hide-panel hidden">
                    <i class="process-icon-toggle-on"></i>
        ...
        <div class="panel-content" style="display:none">
            <div class="form-group">
                <label class="control-label col-lg-3" for="id_currency">
                    {l s='Currency' mod='jxoneclickorder'}
                </label>
                <script type="text/javascript">
            ...
            <div class="form-group">
                <label class="control-label col-lg-3" for="id_lang">
                    {l s='Language' mod='jxoneclickorder'}
                </label>
                <div class="col-lg-9">
        ...
        <div class="panel-content" style="display:none">
            <div id="search_product" class="hidden">
				...
				<span title="" data-toggle="tooltip" class="label-tooltip"
                      data-original-title="{l s='Search for an existing product by typing the first letters of its name.' mod='jxoneclickorder'}">
					{l s='Search for a product' mod='jxoneclickorder'}
				</span>
                    </label>
                    <div class="col-lg-9">
                ...
                <div id="products_found">
                    ...
                    <!-- @TODO: please be kind refactor -->
                    <div class="form-group">
                        <div class="col-lg-9 col-lg-offset-3">
                          <div id="customization_list"></div>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-lg-3" for="qty">{l s='Quantity' mod='jxoneclickorder'}</label>
                        <div class="col-lg-9">
                            <input type="text" name="qty" id="qty" class="form-control fixed-width-sm" value="1"/>
                            <p class="help-block">{l s='In stock' mod='jxoneclickorder'} <span id="qty_in_stock"></span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="col-lg-9 col-lg-offset-3">
                            <button type="button" class="btn btn-default" id="submitAddProduct">
                                ...
                                {l s='Add to cart' mod='jxoneclickorder'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div id="cart_products">
                <div class="row">
                    <div class="col-lg-12">
                        <table class="table" id="customer_cart">
                            <thead>
                            <tr>
                                <th><span class="title_box">{l s='Product' mod='jxoneclickorder'}</span></th>
                                <th><span class="title_box">{l s='Description' mod='jxoneclickorder'}</span></th>
                                <th><span class="title_box">{l s='Reference' mod='jxoneclickorder'}</span></th>
                                <th><span class="title_box">{l s='Unit price' mod='jxoneclickorder'}</span></th>
                                <th><span class="title_box">{l s='Quantity' mod='jxoneclickorder'}</span></th>
                                <th><span class="title_box">{l s='Price' mod='jxoneclickorder'}</span></th>
                            </tr>
                            </thead>
                            <tbody>
                            </tbody>
                ...
                <div class="form-group">
                    <div class="col-lg-12">
                        <div class="alert alert-warning">{l s='The prices are without taxes.' mod='jxoneclickorder'}</div>
                    </div>
                </div>
            </div>
            <div class="panel-buttons clearfix">
                <a href="#" id="add_products" class="btn btn-success pull-right"><i
                            class="icon-plus-sign-alt"></i> {l s='Add Products' mod='jxoneclickorder'}</a>
                <a href="#"
                   class="cancel btn btn-default pull-right hidden">{l s='View cart' mod='jxoneclickorder'}</a>
            </div>
        </div>
    </div>
    ...
    <div class="panel closed success" id="vouchers_part" style="display:none;">
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                ...
                {l s='Vouchers' mod='jxoneclickorder'}
            </div>
            <a href="#" class="toggle-panel-btn">
                <span class="hide-panel hidden">
                    <i class="process-icon-toggle-on"></i>
        ...
        <div class="panel-content" style="display: none;">
            <div class="form-group">
                <label class="control-label col-lg-3">
                    {l s='Search for a voucher' mod='jxoneclickorder'}
                </label>
                <div class="col-lg-9">
                    <div class="row">
                        <div class="col-lg-12">
            ...
            <div class="row">
                <table class="table" id="voucher_list">
                    <thead>
                    <tr>
                        <th><span class="title_box">{l s='Name' mod='jxoneclickorder'}</span></th>
                        <th><span class="title_box">{l s='Description' mod='jxoneclickorder'}</span></th>
                        <th><span class="title_box">{l s='Value' mod='jxoneclickorder'}</span></th>
                        <th></th>
                    </tr>
                    </thead>
                    <tbody>
    ...
    <div class="panel closed {if $cart->id_customer == 0}warning{else}success{/if}" id="address_part"
        ...
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                ...
                {l s='Addresses' mod='jxoneclickorder'}
            </div>
            <a href="#" class="toggle-panel-btn">
                <span class="hide-panel hidden">
                    <i class="process-icon-toggle-on"></i>
        ...
        <div class="panel-content" style="display:none;">
            ...
            <div class="row">
                <div id="address_delivery" class="col-lg-6">
                    <h4>
                        ...
                        {l s='Delivery' mod='jxoneclickorder'}
                    </h4>
                ...
                <div id="address_invoice" class="col-lg-6">
                    <h4>
                        ...
                        {l s='Invoice' mod='jxoneclickorder'}
                    </h4>
                    <div class="row-margin-bottom">
            ...
            <div class="create_address hidden">
                <h4>
                    {l s='Create address' mod='jxoneclickorder'}
                </h4>
                <div class="form-wrapper" id="address_form">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">
                                {l s='Identification Number' mod='jxoneclickorder'}
                            </label>
                            <input type="text" value="" id="dni" name="dni">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='Address alias' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="alias" name="alias">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='First Name' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="{if isset($customer) && $customer->firstname}{$customer->firstname}{/if}" id="firstname" name="firstname">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='Last Name' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="{if isset($customer) && $customer->lastname}{$customer->lastname}{/if}" id="lastname" name="lastname">
                        </div>
                        <div class="form-group">
                            <label class="control-label">
                                {l s='Company' mod='jxoneclickorder'}
                            </label>
                        ...
                        <div class="form-group">
                            <div style="display: visible" id="vat_area">
                                <label class="control-label">
                                    {l s='VAT number' mod='jxoneclickorder'}
                                </label>
                        ...
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='Address' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="address1" name="address1">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">
                                {l s='Address (2)' mod='jxoneclickorder'}
                            </label>
                            <input type="text" class="" value="" id="address2" name="address2">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='City' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="city" name="city">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='Zip/Postal Code' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="postcode" name="postcode">
                        </div>
                        <div class="form-group">
                            <label class="control-label required">
                                {l s='Country' mod='jxoneclickorder'}
                            </label>
                        ...
                        <div class="form-group">
                            <label class="control-label">
                                <sup>**</sup>{l s='Home phone' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="phone" name="phone">
                        </div>
                        <div class="form-group">
                            <label class="control-label">
                                <sup>**</sup>{l s='Mobile phone' mod='jxoneclickorder'}
                            </label>
                            <input type="text" required class="" value="" id="phone_mobile" name="phone_mobile">
                        </div>
                ...
                <div class="btn-wrap">
                    <a href="#" class="btn btn-default save-address pull-left hidden">{l s='Create address' mod='jxoneclickorder'}</a>
                    <a href="#" class="cancel btn btn-default pull-right hidden">{l s='Cancel' mod='jxoneclickorder'}</a>
                </div>
            </div>
            <div class="panel-buttons clearfix">
                <a class="btn btn-success pull-right" id="new_address"
                   href="#">
                    ...
                    {l s='Add a new address' mod='jxoneclickorder'}
                </a>
            </div>
        </div>
    </div>
    <div class="panel closed {if $cart->id_customer == 0}warning{else}success{/if}" id="carriers_part"
        ...
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                <i class="icon-truck"></i>
                {l s='Shipping' mod='jxoneclickorder'}
            </div>
            <a href="#" class="toggle-panel-btn">
                <span class="hide-panel hidden">
                    <i class="process-icon-toggle-on"></i>
        ...
        <div class="panel-content" style="display: none">
            ...
            <div id="carrier_form">
                <div class="form-group">
                    <label class="control-label col-lg-3">
                        {l s='Delivery option' mod='jxoneclickorder'}
                    </label>
                    <div class="col-lg-9">
                ...
                <div class="form-group">
                    <label class="control-label col-lg-3" for="shipping_price">
                        {l s='Shipping price (Tax incl.)' mod='jxoneclickorder'}
                    </label>
                    <div class="col-lg-9">
                ...
                <div class="form-group">
                    <label class="control-label col-lg-3" for="free_shipping">
                        {l s='Free shipping' mod='jxoneclickorder'}
                    </label>
                    ...
                    <div class="form-group">
                        <div class="checkbox col-lg-9 col-offset-3">
                            <label for="carrier_recycled_package">
                                ...
                                {l s='Recycled package' mod='jxoneclickorder'}
                            </label>
                        </div>
                    </div>
                {/if}
                    ...
                    <div class="form-group">
                        <div class="checkbox col-lg-9 col-offset-3">
                            <label for="order_gift">
                                ...
                                {l s='Gift' mod='jxoneclickorder'}
                            </label>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-lg-3"
                               for="gift_message">{l s='Gift message' mod='jxoneclickorder'}</label>
                        <div class="col-lg-9">
    ...
    <div class="panel" id="summary_part" style="display:none;">
        <div class="panel-heading toggle-panel">
            <div class="panel-title">
                ...
                {l s='Summary' mod='jxoneclickorder'}
            </div>
            <a href="#" class="toggle-panel-btn">
                <span class="hide-panel">
                    <i class="process-icon-toggle-on"></i>
        ...
        <div class="panel-content">
                ...
                <div class="row">
                    <div class="col-lg-2">
                        <div class="data-focus">
                            <span>{l s='Total products' mod='jxoneclickorder'}</span><br/>
                            <span id="total_products" class="size_l text-success"></span>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="data-focus">
                            <span>{l s='Total vouchers (Tax excl.)' mod='jxoneclickorder'}</span><br/>
                            <span id="total_vouchers" class="size_l text-danger"></span>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="data-focus">
                            <span>{l s='Total shipping (Tax excl.)' mod='jxoneclickorder'}</span><br/>
                            <span id="total_shipping" class="size_l"></span>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="data-focus">
                            <span>{l s='Total taxes' mod='jxoneclickorder'}</span><br/>
                            <span id="total_taxes" class="size_l"></span>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="data-focus">
                            <span>{l s='Total (Tax excl.)' mod='jxoneclickorder'}</span><br/>
                            <span id="total_without_taxes" class="size_l"></span>
                        </div>
                    </div>
                    <div class="col-lg-2">
                        <div class="data-focus data-focus-primary">
                            <span>{l s='Total (Tax incl.)' mod='jxoneclickorder'}</span><br/>
            ...
            <div class="row">
                <div class="order_message_right col-lg-12">
                    <div class="form-group">
                        <label class="control-label col-lg-3"
                               for="order_message">{l s='Order message' mod='jxoneclickorder'}</label>
                        <div class="col-lg-6">
                            <textarea name="order_message" id="order_message" rows="3" cols="45"></textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-lg-3">{l s='Payment' mod='jxoneclickorder'}</label>
                        <div class="col-lg-9">
                            <select name="payment_module_name" id="payment_module_name">
                                {if !$PS_CATALOG_MODE}
                                    ...
                                    {/foreach}
                                {else}
                                    <option value="boorder">{l s='Back office order' mod='jxoneclickorder'}</option>
                                {/if}
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-lg-3">{l s='Order status' mod='jxoneclickorder'}</label>
                        <div class="col-lg-9">
                    ...
                    <div class="form-group">
                        <div class="col-lg-9 col-lg-offset-3">
                            <button type="submit" name="submitAddOrder" class="btn btn-default" data-id-order="{$order->id_order|escape:'htmlall':'UTF-8'}" data-order-status="created">
                                ...
                                {l s='Create the order' mod='jxoneclickorder'}
                            </button>
                        </div>
                    </div>
                </div>
